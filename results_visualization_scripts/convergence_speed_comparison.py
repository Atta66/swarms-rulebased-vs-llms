"""
Convergence Speed Comparison: Classic ACO vs LLM ACO
Shows convergence speed progression over number of trials
"""
...
def load_results(file_path):
    """Load and extract real convergence speed data from individual trial results."""
    try:
        ...
        for trial in individual_results:
            # Extract actual convergence speed from swarm_performance
            swarm_perf = trial.get('swarm_performance', {})
            speed = swarm_perf.get('convergence_speed', 0)
            convergence_speeds.append(speed)
        
        print(f"✅ Extracted {len(convergence_speeds)} real trial convergence speed values")
...
def create_convergence_speed_comparison():
    """Create comparison visualization for convergence speed."""
    # Load data
    ...
    aco_speeds = load_results(aco_file)
    aco_llm_speeds = load_results(aco_llm_file)
    
    if not aco_speeds or not aco_llm_speeds:
        print("❌ Failed to load convergence speed data")
        return
    
    # Create visualization
    plt.figure(figsize=(12, 8))
    
    trials_aco = range(1, len(aco_speeds) + 1)
    trials_aco_llm = range(1, len(aco_llm_speeds) + 1)
    
    # Plot individual trial results
    plt.scatter(trials_aco, aco_speeds, alpha=0.6, color='lightblue', 
                label='Classic ACO', s=50, edgecolors='darkblue')
    plt.scatter(trials_aco_llm, aco_llm_speeds, alpha=0.6, color='lightcoral', 
                label='LLM ACO', s=50, edgecolors='darkred')
    
    # Add trend lines
    z_aco = np.polyfit(trials_aco, aco_speeds, 1)
    p_aco = np.poly1d(z_aco)
    plt.plot(trials_aco, p_aco(trials_aco), 'darkblue', linewidth=2, linestyle='--', alpha=0.8)
    
    z_aco_llm = np.polyfit(trials_aco_llm, aco_llm_speeds, 1)
    p_aco_llm = np.poly1d(z_aco_llm)
    plt.plot(trials_aco_llm, p_aco_llm(trials_aco_llm), 'darkred', linewidth=2, linestyle='--', alpha=0.8)
    
    # Add mean lines
    aco_mean = np.mean(aco_speeds)
    aco_llm_mean = np.mean(aco_llm_speeds)
    ...
    plt.ylabel('Convergence Speed', fontsize=14)
    ...
    filename = f"convergence_speed_comparison_{timestamp}.png"
    save_path = os.path.join("..", "results", "visualizations", filename)
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"💾 Convergence speed comparison saved to: {save_path}")
    
    plt.show()

if __name__ == "__main__":
    print("⚡ Convergence Speed Comparison: Classic ACO vs LLM ACO")
    print("=" * 60)
