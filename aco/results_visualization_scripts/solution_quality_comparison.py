"""
Solution Quality Comparison: Classic ACO vs LLM ACO
Shows solution quality progression over number of trials
"""
...
def load_results(file_path):
    """Load and extract real solution quality data from individual trial results."""
    try:
        ...
        for trial in individual_results:
            # Extract actual solution quality from swarm_performance
            swarm_perf = trial.get('swarm_performance', {})
            quality = swarm_perf.get('solution_quality', 0)
            solution_qualities.append(quality)
        
        print(f"✅ Extracted {len(solution_qualities)} real trial solution quality values")
...
def create_solution_quality_comparison():
    """Create comparison visualization for solution quality."""
    # Load data
    ...
    aco_qualities = load_results(aco_file)
    aco_llm_qualities = load_results(aco_llm_file)
    
    if not aco_qualities or not aco_llm_qualities:
        print("❌ Failed to load solution quality data")
        return
    
    # Create visualization
    plt.figure(figsize=(12, 8))
    
    trials_aco = range(1, len(aco_qualities) + 1)
    trials_aco_llm = range(1, len(aco_llm_qualities) + 1)
    
    # Plot individual trial results
    plt.scatter(trials_aco, aco_qualities, alpha=0.6, color='lightblue', 
                label='Classic ACO', s=50, edgecolors='darkblue')
    plt.scatter(trials_aco_llm, aco_llm_qualities, alpha=0.6, color='lightcoral', 
                label='LLM ACO', s=50, edgecolors='darkred')
    
    # Add trend lines
    z_aco = np.polyfit(trials_aco, aco_qualities, 1)
    p_aco = np.poly1d(z_aco)
    plt.plot(trials_aco, p_aco(trials_aco), 'darkblue', linewidth=2, linestyle='--', alpha=0.8)
    
    z_aco_llm = np.polyfit(trials_aco_llm, aco_llm_qualities, 1)
    p_aco_llm = np.poly1d(z_aco_llm)
    plt.plot(trials_aco_llm, p_aco_llm(trials_aco_llm), 'darkred', linewidth=2, linestyle='--', alpha=0.8)
    
    # Add mean lines
    aco_mean = np.mean(aco_qualities)
    aco_llm_mean = np.mean(aco_llm_qualities)
    ...
    filename = f"solution_quality_comparison_{timestamp}.png"
    save_path = os.path.join("..", "results", "visualizations", filename)
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"💾 Solution quality comparison saved to: {save_path}")
    
    plt.show()

if __name__ == "__main__":
    print("📊 Solution Quality Comparison: Classic ACO vs LLM ACO")
    print("=" * 60)
    create_solution_quality_comparison()
